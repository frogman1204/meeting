function out = run_pack(mode, varargin)
%RUN_PACK Grouped run_* functions.

switch lower(mode)
    case 'core'
        out = run_all_figures_core_local(varargin{1}, varargin{2});
    case 'sweep_power'
...
function out_dir = run_all_figures_core_local(params, mode_name)
rng(params.rng_seed);
...
for ix=1:nx
    ...
    if strcmp(kind,'rho')
        rho_fixed=xvec(ix); rho_grid=p.rho_grid(p.rho_grid<=xvec(ix)); if isempty(rho_grid), rho_grid=xvec(ix); end
    end
    tmp=zeros(p.numMC,ns,12);
    for imc=1:p.numMC
        ch=apply_pack('generate_channels');
        ch=apply_pack('apply_csi_error',ch,csi);
        ch=apply_pack('apply_blockage_effect',ch,blk);
        sols={solve_pack('pure_noma',ch,Pt,sic,p.sigma2,0,p.rate_threshold), ...
              solve_pack('noma_fixed',ch,Pt,sic,p.sigma2,rho_fixed,p.rate_threshold), ...
              solve_pack('noma_opt',ch,Pt,sic,p.sigma2,rho_grid,p.rate_threshold), ...
              solve_pack('pure_rsma',ch,Pt,sic,p.sigma2,0,p.rate_threshold), ...
              solve_pack('rsma_fixed',ch,Pt,sic,p.sigma2,rho_fixed,p.rate_threshold), ...
              solve_pack('rsma_opt',ch,Pt,sic,p.sigma2,rho_grid,p.rate_threshold)};
        for is=1:ns
...
function x=item(name,fig)
x=struct('name',name,'fig',fig);
end
