...
if nargin < 8, rsma_cfg = struct(); end
R1 = rsma_cfg.R1; R2 = rsma_cfg.R2;
rs = rsma_cfg;
...
met.Pc = rs.Pc; met.P1 = rs.P1; met.P2 = rs.P2;
met.mu = rs.mu;
...
met.ber_tag = core('tag_ber', ch, rs.P1, rs.P2, rs.Pc, rho, ambc_cfg);
end

function met = local_rsma_opt(ch, Pt, sic_err, sigma2, rho_grid, rate_threshold, harvest_cfg, ambc_cfg)
p_step = local_get_or(harvest_cfg, 'rsma_p_step', 0.1); %#ok<NASGU>
best = opt('rsma_wmmse_zeta', ch, Pt, sic_err, sigma2, rho_grid, harvest_cfg, ambc_cfg);
met = local_rsma_eval(ch, Pt, sic_err, sigma2, best.zeta, rate_threshold, best.zeta, best, ambc_cfg);
...
if rsma_pack_call_count <= 3 || mod(rsma_pack_call_count, 100) == 0
    ...
    fprintf(['[rsma_wmmse] call=%d | zeta=%.3f | alpha_c=%.3f alpha_1=%.3f alpha_2=%.3f | ' ...
