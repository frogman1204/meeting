function res = run_sweep_sic(p)
%RUN_SWEEP_SIC Sweep SIC imperfection.
...
for ix=1:nx
 sic=xvec(ix); tmp=zeros(p.numMC,ns,12);
 for imc=1:p.numMC
...
res.x_values=xvec; res.scheme_names=p.scheme_names; res.sweep_name='sic'; res.x_label='SIC imperfection';
end
